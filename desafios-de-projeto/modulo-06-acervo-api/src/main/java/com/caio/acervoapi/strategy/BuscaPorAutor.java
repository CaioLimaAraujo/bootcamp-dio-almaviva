...
@Component
public class BuscaPorAutor implements BuscaStrategy {

    private final ObraRepository repository;

    public BuscaPorAutor(ObraRepository repository) {
        this.repository = repository;
    }

    @Override
    public List<Obra> buscar(String valor) {
        return repository.findByAutorContainingIgnoreCase(valor);
    }
}