# 📁 Padronização de Nomes de Projetos em TI

## 📌 Apresentação e Contextualização

Esse foi o primeiro Desafio de Código do módulo **Primeiros Passos na Sintaxe Java**, parte do **Bootcamp Backend Java & QA** — uma parceria entre a **DIO (Digital Innovation One)** e a **Almaviva**.
...
- `Main` — nome escolhido para a classe. Em Java, o nome da classe deve ser idêntico ao nome do arquivo (neste caso, `Main.java`).
...
Declara o método principal da aplicação — o ponto de entrada que a JVM (Máquina Virtual Java) executa primeiro.

- `public` — o método precisa ser acessível pela JVM, então deve ser público.
...
⚫ **Linhas 5 a 12 — Bloco principal: leitura, processamento e saída**
...
- `new Scanner(...)` — instancia (cria) um novo objeto dessa classe.
- `System.in` — argumento que indica a fonte de leitura: a entrada padrão do sistema, ou seja, o teclado.

A analogia útil: `System.in` é o cano bruto de dados vindo do teclado; `Scanner` é a ferramenta que interpreta esses dados e os entrega de forma legível.
...
- `scanner.nextLine()` — método do objeto `scanner` que lê uma linha inteira da entrada, incluindo espaços, até o usuário pressionar Enter.
...
Cria uma variável do tipo inteiro e a preenche com o número total de caracteres de `nomeProjeto`.

- `int` — tipo de dado para números inteiros.
- `comprimento` — nome escolhido para a variável.
- `length()` — método da classe `String` que retorna a quantidade de caracteres da string, incluindo espaços, números e símbolos.

---

**Linha 12 — `System.out.println(nomeEmMaiusculas + " " + comprimento);`**

Imprime o resultado na tela no formato exigido pelo desafio: nome em maiúsculas, um espaço, e o número de caracteres.

- `System.out` — objeto responsável pela saída padrão (o terminal).
- `println()` — método que imprime o texto e quebra a linha ao final.
- O operador `+` concatena (une) os três elementos: a string em maiúsculas, um espaço literal `" "` e o valor inteiro do comprimento.
...
É uma boa prática sempre fechar o `Scanner` após o uso para evitar vazamento de recursos.

---

## 📝 Relatório Pessoal

Esse desafio foi minha introdução prática à sintaxe básica do Java dentro do Bootcamp Backend Java & QA. Através dele pude exercitar e consolidar os seguintes conhecimentos:

- A estrutura obrigatória de toda aplicação Java: importação de classes, declaração de classe e método `main`.
- A distinção entre **classe** e **objeto**: `Scanner` é a classe; `scanner` é o objeto criado a partir dela.
- O uso de métodos da classe `String` para manipulação de texto: `toUpperCase()` para converter maiúsculas e `length()` para contar caracteres.
- A diferença entre **função** e **método**: em Java, todo bloco de código pertence a uma classe e, portanto, tecnicamente é um método.
- A importância de fechar recursos como o `Scanner` após o uso.

Foi um exercício simples, mas que deixou clara a rigidez e organização que o Java exige desde o primeiro código.
